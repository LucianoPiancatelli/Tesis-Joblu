...
// ==========================================
// 🛠️ MIDDLEWARES GLOBALES DE LOGS
// ==========================================
app.use((req, res, next) => {
  console.log(`[LOG] ${req.method} ${req.url} - Origin: ${req.headers.origin}`);
  const logMsg = `[${new Date().toISOString()}] ${req.method} ${req.url} - Origin: ${req.headers.origin}\n`;
  fs.appendFileSync(path.join(__dirname, "access_logs.txt"), logMsg);
  next();
});
...
app.use((err, req, res, next) => {
  ...
  fs.appendFileSync(path.join(__dirname, "error_logs.txt"), errorLog);
